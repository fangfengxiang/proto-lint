...
[![CI](https://github.com/fangfengxiang/php-proto-lint/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/fangfengxiang/php-proto-lint/actions/workflows/ci.yml)
[![Static Analysis](https://github.com/fangfengxiang/php-proto-lint/actions/workflows/static-analysis.yml/badge.svg?branch=main)](https://github.com/fangfengxiang/php-proto-lint/actions/workflows/static-analysis.yml)
[![PHP](https://img.shields.io/badge/PHP-%3E%3D8.2-777BB4.svg)](https://www.php.net/)
[![License](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](https://www.apache.org/licenses/LICENSE-2.0)
[![Latest Release](https://img.shields.io/github/v/release/fangfengxiang/php-proto-lint)](https://github.com/fangfengxiang/php-proto-lint/releases)

> Schema-First PHP 协议治理与一致性 Lint 引擎

`php-proto-lint` 是一个 PHP CLI 工具，充当 `.proto` 契约与 PHP 源码之间的静态一致性校验引擎。它通过三套命令实现协议治理闭环：`check`（契约一致性检查）、`shadow-lint`（影子流量审计）、`inject-attributes`（注解安全注入）。
...
```bash
composer global require php-proto-lint/php-proto-lint
```

### 项目安装

```bash
composer require --dev php-proto-lint/php-proto-lint
```

### 源码安装

```bash
git clone https://github.com/fangfengxiang/php-proto-lint.git
cd php-proto-lint
composer install
```
...
# 3. 检查契约一致性
./bin/php-proto-lint check --config="proto-bulk.json"
```
...
```json
{
    "$schema": "https://openspec.dev/php-proto-lint/v1/bulk.json",
    "source_dir": "src/Service/",
...
```json
{
    "$schema": "https://openspec.dev/php-proto-lint/v1/mapping.json",
...
```bash
./bin/php-proto-lint shadow-lint --config="proto-mapping.json"
```
...
./bin/php-proto-lint inject-attributes --config="proto-bulk.json"

# PHP 7 模式（定界符沙箱）
./bin/php-proto-lint inject-attributes --config="proto-bulk.json" --php7

# 预览模式（不写入文件）
./bin/php-proto-lint inject-attributes --config="proto-bulk.json" --dry-run
```
...
- name: Run php-proto-lint check
  run: ./bin/php-proto-lint check --config="proto-bulk.json"
```
...
[Apache-2.0](LICENSE) © php-proto-lint
